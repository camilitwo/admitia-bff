...
public interface NotificationRepository extends JpaRepository<NotificationEntity, Long> {
    @Query("select n from NotificationEntity n where (:recipientType is null or n.recipientType = :recipientType) and (:recipientId is null or n.recipientId = :recipientId) and (:channel is null or n.channel = :channel) and (:type is null or n.type = :type) and (:status is null or n.status = :status)")
